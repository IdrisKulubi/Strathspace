...
// Use the Web Performance API (available in both browser and Node.js 16+)
...
class PerformanceMonitor {
  ...
  getStats(): PerformanceStats {
    ...
    const errorRate =
      recentMetrics.length > 0
        ? ((recentMetrics.length - successfulQueries.length) /
...
export class MemoryMonitor {
  ...
  static getMemoryStats(): {
    ...
    const average =
      this.measurements.reduce((sum, m) => sum + m.usage, 0) /
    ...
    const firstAvg =
      firstHalf.reduce((sum, m) => sum + m.usage, 0) / firstHalf.length;
    const secondAvg =
      secondHalf.reduce((sum, m) => sum + m.usage, 0) / secondHalf.length;
